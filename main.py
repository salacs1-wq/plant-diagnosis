...
app = FastAPI(
    ...
    description=(
        "PlantNet proxy API.\n"
        "A diseases endpoint kódjai (pl. SEPTTR) nem hivatalos kórtani azonosítók, csak jelzések."
    ),
)
...
def _compact_disease_response(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    top1 = float(best_score) if best_score is not None else None
    best_hu = f"{best} – (kód, fordítás nincs)" if best and _is_code_like(best) else (best or "ismeretlen")
...
def _empty_issue(case_type: str) -> Dict[str, Any]:
    return {
        "bestMatch": "n/a",
        "bestMatch_hu": "nincs (weed mód)" if case_type == "weed" else "nincs megbízható találat",
...
async def _diagnose_core(*, images: List[Tuple[str, bytes, str]], project: str, organs: Optional[str], mode: str, case_type: str) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "caseType": case_type,
        "imageCount": len(images),
        "notes": ["A kódok nem hivatalosak; jelzés/döntéstámogatás."],
    }
    return {"plant": plant, "diseaseOrPest": issue, "summary": summary}
