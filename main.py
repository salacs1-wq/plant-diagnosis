...
# CONFIG
# =========================================================
APP_VERSION = "1.2.6"  # fix verzió, ne env-ből
...
# Pest módban: ha az issueScore ez alatt van, elhallgatjuk a "kódlistát" (zajcsökkentés)
...
def _normalize_organs_for_images(organs: Optional[str], image_count: int) -> Optional[List[str]]:
    """
    Alapból NEM küldünk organs-t.
    Csak akkor küldjük, ha explicit meg van adva és nem auto/none/null.
    """
    if organs is None:
        return None
...
def _compact_disease_response(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for r in results[:10]:
        ...
        top_matches.append(
            {"name": name, "name_hu": hu, "score": float(score) if score is not None else None}
        )
...
async def _diagnose_core(*, images: List[Tuple[str, bytes, str]], project: str, organs: Optional[str], mode: str, case_type: str) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "caseType": case_type,
        "imageCount": len(images),
        "notes": ["Kódkimenet nem hivatalos. Pest mód: zajszűrés aktív."],
    }
    return {"plant": plant, "diseaseOrPest": issue, "summary": summary}

# =========================================================
# OpenAI file refs endpoint (GPT Actions)
# =========================================================
